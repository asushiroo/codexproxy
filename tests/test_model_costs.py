...
class ModelCostsTests(unittest.TestCase):
    ...
    def test_load_model_costs_supports_decimal_costs(self) -> None:
        with TemporaryDirectory() as temp_dir:
            model_costs_path = Path(temp_dir) / "model-costs.json"
            model_costs_path.write_text(
                json.dumps({"gpt-5.5": 1.6, "other": 0.5}),
                encoding="utf-8",
            )

            model_costs = load_model_costs(Path(temp_dir))

            self.assertEqual(model_costs["gpt-5.5"], 1.6)
            self.assertEqual(model_costs["other"], 0.5)
